...
import './network-nvr-index.less'
import { NetworkNVRIndexMessage } from './network-nvr-index.message'

export namespace NetworkNVRIndex {
  export class HtmlController {
    ...
    private index = LocalStorageService.navigation.network.nvr.get()
    ...
    message = new NetworkNVRIndexMessage(this.element.iframe)
      ...
      if (this.element.items) {
        ...
        if (index >= 0) {
          ...
          if (this.element.iframe) {
            ...
            this.index = index
            LocalStorageService.navigation.network.nvr.save(this.index)
          }
        }
      }
    }
    ...
    private factory(index: number): string {
      switch (index) {
        case 0:
          return '../network-nvr-config/network-nvr-config.html'
        default:
          return ''
      }
    }
