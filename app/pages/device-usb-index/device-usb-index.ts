...
import './device-usb-index.less'

export namespace DeviceUsbIndex {
  export class HtmlController {
    ...
    private index = LocalStorageService.navigation.device.usb.get()
      ...
      if (this.element.items) {
        ...
        if (index >= 0) {
          ...
          if (this.element.iframe) {
            ...
            this.index = index
            LocalStorageService.navigation.device.usb.save(this.index)
          }
        }
      }
    }
    ...
    private factory(index: number): string {
      switch (index) {
        case 0:
          return '../device-usb-info/device-usb-info.html'
        default:
          return ''
      }
    }
